...
mat3 Transform::rotate(const float degrees, const vec3& axis) {
	const vec3 axisNorm = glm::normalize(axis);
	...
	return	_oneMinusCos*mat3(axisNorm[0]*axisNorm, axisNorm[1]*axisNorm, axisNorm[2]*axisNorm)
	+		_cos*mat3()
	+		_sin*mat3(glm::cross(axisNorm, id[0]), glm::cross(axisNorm, id[1]), glm::cross(axisNorm, id[2]));
}
...
mat4 Transform::lookAt(vec3 eye, vec3 up) {
  ...
  // apply a rotation such that 'x' goes to u, 'y' to v and 'z' to w
